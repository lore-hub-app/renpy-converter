import { LoreHubJsonNode } from "../dto/LoreHubJson";
...
export default class DialogNode {
  ...
  toRenpyLine(characters: Array<Character> = []): string {
    ...
    if (
      this.contents[0] instanceof ContentReference &&
      ...
      const neededCharacter = characters.find(
        (c) => c.id === asRef.document_id
      );
      if (neededCharacter == null)
        throw new Error(
          `Cannot find character/document with id ${asRef.document_id}`
        );
    ...
    result.push('"');
    for (let i = 0; i < neededContent.length; i += 1) {
      const content = neededContent[i];
      result.push(`${content.text}`);
      if (i + 1 !== neededContent.length) result.push("\n");
    }
    result.push('"');
    result.push("\n");
    return result.join("");
